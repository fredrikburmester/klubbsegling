...
async function removeBoat(id, el) {
    await axios.post('/users/removeboat/' + id, {})
...
async function addBoat(form) {
    await axios.post('/users/addboat/' + form.ref.value, {})
        .then(function (response) {
            if (response.data.response.success == "true") {
                window.createNotification({
                    closeOnClick: false,
...
async function addCrewMember(boat_id, crewMember_id, el) {
    ...
    if(errors.length == 0) {
        if (boat_id !== undefined && boat_id !== null && boat_id.length > 0) {
            if (crewMember_id !== undefined && crewMember_id !== null && crewMember_id.length > 0) {

                await axios.post('/boat/add/sailor', {
                ...
                .then(function (response) {
                    if (response.status == 200) {
                        ...
                        boats.forEach(boat => {
                            if (boat._id == boat_id && res.name.length > 0 && res.id.length > 0) {
                                boat.crew.push({
                                    id: res.id
                                })
                            }
...
async function removeCrewMember(boat_id, crewMember_id, el) {
    await axios.post('/boat/remove/sailor', {
            user_id: crewMember_id,
            boat_id: boat_id
        })
        .then(function (response) {
...
async function getUserData() {
    await axios.post('/get-user').then(response => {
        if (response.status == 200)
...
async function unRegisterForRace(id, el) {
    await axios.post('/un-register-for-race/' + id).then(response => {
        if (response.status == 200) {
            window.createNotification({
                closeOnClick: false,
                displayCloseButton: false,
...
async function registerForRace(id, boat_id, button_id) {
    ...
    if (boat_id === undefined || boat_id == null || boat_id.length == 0) {
        ...
        await axios.post('/register-for-race', {
            id,
            boat_id
        }).then(response => {
...
async function createRaceCard(race) {
    ...
    cardDate.innerHTML = `${race.startDate.split("T")[0].replace('-', "/")} - ${race.endDate.split("T")[0].replace('-', "/")}`
