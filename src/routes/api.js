...
router.get(
	'/:name(serie|handicap|race|club|checkpoint|boat|user)?/:id',
	async function (req, res) {
		if (req.params.name == 'serie')
			var object = await Serie.find({
				_id: req.params.id,
			})
		else if (req.params.name == 'handicap')
			var object = await Handicap.find({
				_id: req.params.id,
			})
			...
			var object = await Club.find({
				_id: req.params.id,
			})
		else if (req.params.name == 'checkpoint')
			var object = await Checkpoint.find({
				_id: req.params.id,
			})
		else if (req.params.name == 'boat')
			var object = await Boat.find({
				_id: req.params.id,
			})
...
router.post('/find/races', async (req, res) => {
	...
	if (typeof clubs == 'string' && clubs.length == 0) check_clubs = false
	...
	if (typeof series == 'string' && series.length == 0) check_series = false
	if (typeof race_names == 'string' && race_names.length == 0)
		check_race_names = false
	if (typeof future == 'string' && future.length == 0) check_future = false
	if (typeof previous == 'string' && previous.length == 0)
		check_previous = false
	if (typeof years == 'string' && years.length == 0) check_years = false

	if (typeof years === 'number') var years = years.toString()
	if (typeof race_ids === 'number') var race_ids = race_ids.toString()
	if (typeof crew_members === 'number')
		var crew_members = crew_members.toString()
	if (typeof participants === 'number')
		var participants = participants.toString()
	...
	await Race.find()
		.then((response) => {
			response.forEach((race) => {
					...
					if (crew_members.length == 0) {
						...
						if (boat_ids_with_crew_members.length > 0) {
							boat_ids_with_crew_members.forEach((boat_id) => {
								if (race.participants.includes(boat_id))
									included = true
							})
						}
						if (!included) return
				...
				if (check_race_ids) if (!race_ids.includes(race._id)) return

				if (check_clubs) if (!clubs.includes(race.club)) return

				if (check_series) if (!series.includes(race.serie)) return
				...
				if (check_years)
					if (
						!years.includes(race.startDate.getFullYear().toString())
					)
						return
...
router.post('/find/users', async (req, res) => {
	var { race_ids, clubs, user_ids, user_names, boat_ids, boat_names } =
		req.body
...
router.post('/find/boats', async (req, res) => {
	var { user_ids, boat_ids, race_ids, boat_names, owner_names } = req.body
		...
		.then((response) => {
			response.forEach((boat) => {
				...
				if (
					boat_ids !== undefined &&
					boat_ids.length > 0 &&
					!boat_ids.includes(boat._id.toString())
				) {
					return
				}
				...
				if (
					boat_names !== undefined &&
					boat_names.length > 0 &&
					!boat_names.includes(boat.name)
				) {
					return
				}
				if (
					owner_names !== undefined &&
					owner_names.length > 0 &&
					!owner_names.includes(boat.owner)
				) {
					return
				}
				result.push(boat)
