...
router.post(
	'/add/:obj(serie|handicap|race|club|checkpoint)?',
	ensureAuthenticated,
	async (req, res) => {
		const {
			name,
		...
		if (req.params.obj == 'checkpoint') {
			...
			if (
				...
				if (!req.files || Object.keys(req.files).length === 0) {
					...
					logo.mv(
						'./public/uploads/logos/' + shortName + '/' + filename,
						function (err) {
							if (err) {
								errors.push({
...
router.post(
	'/remove/:name(serie|handicap|race|club|checkpoint)?',
	ensureAuthenticated,
	async (req, res) => {
		const { id } = req.body
